...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-fluid page-header py-5">
            <h1 class="text-center text-white display-6"><MARQUEE direction="right">Vue D'ensemble des Comptes</MARQUEE></h1>
...
// Récupérer les données depuis la base de données
$sql = "SELECT id, prenom, nom, telephone, email, role, photo FROM utilisateurs";
$result = $conn->query($sql);
...
<div class="container-fluid py-4">
    <div class="row justify-content-center mb-3">
        <div class="col-auto">
            ...
            <span class="badge bg-secondary">Nombre total d'utilisateurs</span>
            <span class="badge bg-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Total: <?php echo count($utilisateurs); ?>"><?php echo count($utilisateurs); ?></span>
        </div>
    </div>
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-3">
        ...
        <div class="col">
            <div class="card h-100">
                <img src="uploads/<?php echo htmlspecialchars($utilisateur['photo']); ?>" class="card-img-top rounded-circle" alt="Photo de profil" style="width: 100px; height: 100px; object-fit: cover;">
                <div class="card-body">
                    <h5 class="card-title"><?php echo htmlspecialchars($utilisateur['prenom'] . ' ' . $utilisateur['nom']); ?></h5>
                    <p class="card-text">Téléphone: <?php echo htmlspecialchars($utilisateur['telephone']); ?></p>
                    <p class="card-text">Email: <?php echo htmlspecialchars($utilisateur['email']); ?></p>
                    <p class="card-text">Rôle: <?php echo htmlspecialchars($utilisateur['role']); ?></p>
                </div>
                <div class="card-footer d-flex justify-content-center">
                    <!-- Button to open the edit modal -->
                    <button type="button" class="btn btn-sm btn-secondary me-2" data-bs-toggle="modal" data-bs-target="#editModal<?php echo $utilisateur['id']; ?>">
                        <i class="fa-solid fa-pen-to-square text-white"></i>
                    </button>
                    <!-- Button to open the delete confirmation modal -->
                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal<?php echo $utilisateur['id']; ?>">
        ...
        <div class="modal fade" id="editModal<?php echo $utilisateur['id']; ?>" tabindex="-1" aria-labelledby="editModalLabel<?php echo $utilisateur['id']; ?>" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalLabel<?php echo $utilisateur['id']; ?>">Modifier Utilisateur</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form method="POST" action="modifier_utilisateur.php" enctype="multipart/form-data">
                            <input type="hidden" name="id" value="<?php echo $utilisateur['id']; ?>">
                            <img src="uploads/<?php echo htmlspecialchars($utilisateur['photo']); ?>" alt="Photo de profil">
                            <div class="mb-3">
                                <label for="prenom<?php echo $utilisateur['id']; ?>" class="form-label">Prénom</label>
                                <input type="text" class="form-control" id="prenom<?php echo $utilisateur['id']; ?>" name="prenom" value="<?php echo htmlspecialchars($utilisateur['prenom']); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="nom<?php echo $utilisateur['id']; ?>" class="form-label">Nom</label>
                                <input type="text" class="form-control" id="nom<?php echo $utilisateur['id']; ?>" name="nom" value="<?php echo htmlspecialchars($utilisateur['nom']); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="telephone<?php echo $utilisateur['id']; ?>" class="form-label">Téléphone</label>
                                <input type="text" class="form-control" id="telephone<?php echo $utilisateur['id']; ?>" name="telephone" value="<?php echo htmlspecialchars($utilisateur['telephone']); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="email<?php echo $utilisateur['id']; ?>" class="form-label">Email</label>
                                <input type="email" class="form-control" id="email<?php echo $utilisateur['id']; ?>" name="email" value="<?php echo htmlspecialchars($utilisateur['email']); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="role<?php echo $utilisateur['id']; ?>" class="form-label">Rôle</label>
                                <input type="text" class="form-control" id="role<?php echo $utilisateur['id']; ?>" name="role" value="<?php echo htmlspecialchars($utilisateur['role']); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="photo<?php echo $utilisateur['id']; ?>" class="form-label">Photo</label>
                                <input type="file" class="form-control" id="photo<?php echo $utilisateur['id']; ?>" name="photo">
                            </div>
                            <div class="mb-3">
                                <label for="password<?php echo $utilisateur['id']; ?>" class="form-label">Nouveau mot de passe</label>
                                <input type="password" class="form-control" id="password<?php echo $utilisateur['id']; ?>" name="password">
                            </div>
...
<div class="modal fade" id="deleteModal<?php echo $utilisateur['id']; ?>" tabindex="-1" aria-labelledby="deleteModalLabel<?php echo $utilisateur['id']; ?>" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteModalLabel<?php echo $utilisateur['id']; ?>">Confirmer la suppression</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
